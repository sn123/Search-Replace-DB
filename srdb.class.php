...
class icit_srdb {
	 ...
	 */
	public function recursive_unserialize_replace( $from = '', $to = '', $data = '', $serialised = false, $json = false ) {
		...
		try {
			...
			elseif (is_string($data) && $this->is_json($data)) {
					...
					$data = $this->recursive_unserialize_replace( $from, $to, $unserialized, false, true);
			}
			elseif ( is_array( $data ) ) {
				$_tmp = array( );
				foreach ( $data as $key => $value ) {
					$_tmp[ $key ] = $this->recursive_unserialize_replace( $from, $to, $value, false, false);
					// if ($key === 'image_url') {
						// $this->add_error($_tmp[$key], 'results' );
					// }
				}
			...
			if ($json) {
				// $this->add_error(json_encode($data), 'results');
